...
class ProcessorMixin(PushToHubMixin):
    ...
    def apply_chat_template(
        ...
        **kwargs: Unpack[AllKwargsForChatTemplate],
        ...
        if tokenize:
            batch_images, batch_videos, batch_panoramic_images = [], [], []
            batch_audios = []
            for conversation in conversations:
                images, videos, panoramic_images = [], [], []
                for message in conversation:
                    visuals = [content for content in message["content"] if content["type"] in ["image", "video", "panoramic_image"]]
                    audio_fnames = [
                        content[key]
                    ...
                    panoramic_image_fnames = [
                        vision_info[key]
                        for vision_info in visuals
                        for key in ["panoramic_image", "url", "path"]
                        if key in vision_info and vision_info["type"] == "panoramic_image"
                    ]
                    panoramic_images.extend(panoramic_image_fnames)
                ...
                batch_panoramic_images.append(panoramic_images)
        ...
        if tokenize:
            ...
            panoramic_images_exist = any((pano is not None) for pano_list in batch_panoramic_images for pano in pano_list)
            out = self(
                text=prompt,
                images=batch_images if images_exist else None,
                videos=batch_videos if videos_exist else None,
                panoramic_images=batch_panoramic_images if panoramic_images_exist else None,
