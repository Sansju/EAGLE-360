...
class Qwen3VLProcessingInfo(Qwen2VLProcessingInfo):
    ...
    def get_supported_mm_limits(self) -> Mapping[str, Optional[int]]:
        return {"image": None, "video": None, "panoramic_image": None}
...
class Qwen3VLDummyInputsBuilder(BaseDummyInputsBuilder[Qwen3VLProcessingInfo]):
    ...
    def get_dummy_text(self, mm_counts: Mapping[str, int]) -> str:
        ...
        num_panoramic_images = mm_counts.get("panoramic_image", 0)

        image_token = "<|vision_start|><|image_pad|><|vision_end|>"
        video_token = "<|vision_start|><|video_pad|><|vision_end|>"
        panoramic_token = "<|vision_start|><|panoramic_image_pad|><|vision_end|>"

        return image_token * num_images + video_token * num_videos + panoramic_token * num_panoramic_images
    ...
    def get_dummy_mm_data(
        ...
        # Add panoramic_image dummy data if needed
        if num_panoramic_images > 0:
            result["panoramic_image"] = self._get_dummy_images(
                width=target_width,
                height=target_height,
                num_images=num_panoramic_images)
        return result
...
class Qwen3VLMultiModalProcessor(BaseMultiModalProcessor[Qwen3VLProcessingInfo]
    ...
    def _call_hf_processor(
        ...
        # Handle panoramic_image separately - process like images but track grid separately
        # Note: get_processor_data() returns "panoramic_images" (plural) as the key
        panoramic_key = "panoramic_images" if "panoramic_images" in mm_data else "panoramic_image"
        if (panoramic_key in mm_data and isinstance(mm_data[panoramic_key], list)
                and len(mm_data[panoramic_key]) > 0):
            panoramic_grid_thw_lst = []
            pixel_values_panoramic_lst = []
            
            for item_idx, pano_image in enumerate(mm_data.pop(panoramic_key, [])):
                # Process each panoramic image using HF processor as a regular image
                pano_mm_data = {"images": [pano_image]}
                pano_outputs = super()._call_hf_processor(
                    prompt="<|vision_start|><|image_pad|><|vision_end|>",
                    mm_data=pano_mm_data,
                    mm_kwargs=mm_kwargs,
                    tok_kwargs=tok_kwargs,
                )
                # Store as panoramic data
                panoramic_grid_thw_lst.append(pano_outputs["image_grid_thw"])
                pixel_values_panoramic_lst.append(pano_outputs["pixel_values"])
            
            panoramic_outputs = dict(
                pixel_values_panoramic=torch.cat(pixel_values_panoramic_lst),
                panoramic_grid_thw=torch.cat(panoramic_grid_thw_lst),
            )
        else:
            panoramic_outputs = dict()
    ...
    def _get_mm_fields_config(
        ...
        # Add panoramic image support
        panoramic_grid_thw = hf_inputs.get("panoramic_grid_thw", torch.empty((0, 3)))
        panoramic_grid_sizes = panoramic_grid_thw.prod(-1)
        ...
        config_dict = dict(
            ...
            # Always include panoramic fields for consistency
            pixel_values_panoramic=MultiModalFieldConfig.flat_from_sizes(
                "panoramic_image", panoramic_grid_sizes),
            panoramic_grid_thw=MultiModalFieldConfig.batched("panoramic_image"),
        )
    ...
    def _get_prompt_updates(
        ...
        def get_panoramic_replacement_qwen3vl(item_idx: int):
            out_item = out_mm_kwargs["panoramic_image"][item_idx]
            grid_thw = out_item["panoramic_grid_thw"].data
            assert isinstance(grid_thw, torch.Tensor)

            num_tokens = int(grid_thw.prod()) // merge_length
            # Use panoramic_image_token_id if available, else fall back to image_token_id
            panoramic_token_id = getattr(hf_config, 'panoramic_image_token_id', None)
            if panoramic_token_id is None:
                panoramic_token_id = hf_processor.image_token_id
            return [panoramic_token_id] * num_tokens
        ...
        # Add panoramic image replacement if panoramic_image modality exists
        if "panoramic_image" in out_mm_kwargs:
            panoramic_target = "<|vision_start|><|panoramic_image_pad|><|vision_end|>"
            prompt_updates.append(
                PromptReplacement(
                    modality="panoramic_image",
                    target=panoramic_target,
                    replacement=get_panoramic_replacement_qwen3vl,
                ),
            )
...
class Qwen3VLForConditionalGeneration(nn.Module, SupportsMultiModal,
    ...
    def get_placeholder_str(cls, modality: str, i: int) -> Optional[str]:
        if modality.startswith("panoramic"):
            return "<|vision_start|><|panoramic_image_pad|><|vision_end|>"
        if modality.startswith("image"):
            return "<|vision_start|><|image_pad|><|vision_end|>"
        if modality.startswith("video"):
            return "<|vision_start|><|video_pad|><|vision_end|>"

        raise ValueError("Only image, panoramic_image, or video modality is supported")
    ...
    def _parse_and_validate_panoramic_input(
            self, **kwargs: object) -> Optional[Qwen2_5_VLImageInputs]:
        """Parse and validate panoramic image input - uses same format as image."""
        pixel_values_panoramic = kwargs.pop("pixel_values_panoramic", None)
        panoramic_grid_thw = kwargs.pop("panoramic_grid_thw", None)

        if pixel_values_panoramic is None:
            return None

        pixel_values_panoramic = self._validate_and_reshape_mm_tensor(
            pixel_values_panoramic, "panoramic image pixel values")
        panoramic_grid_thw = self._validate_and_reshape_mm_tensor(
            panoramic_grid_thw, "panoramic image grid_thw")

        if not isinstance(pixel_values_panoramic, (torch.Tensor, list)):
            raise ValueError("Incorrect type of panoramic image pixel values. "
                             f"Got type: {type(pixel_values_panoramic)}")

        # Return as image input format since processing is identical
        return Qwen2_5_VLImagePixelInputs(type="pixel_values",
                                          pixel_values=pixel_values_panoramic,
                                          image_grid_thw=panoramic_grid_thw)
    ...
    def _parse_and_validate_multimodal_inputs(self, **kwargs: object) -> dict:
        ...
        for input_key in kwargs:
            ...
            if input_key in ("pixel_values_panoramic",
                             ) and "panoramic_image" not in mm_input_by_modality:
                mm_input_by_modality[
                    "panoramic_image"] = self._parse_and_validate_panoramic_input(**kwargs)
    ...
    def get_multimodal_embeddings(
        ...
        for modality in mm_input_by_modality:
            ...
            if modality == "panoramic_image":
                # Process panoramic images the same way as regular images
                panoramic_embeddings = self._process_image_input(multimodal_input)
                multimodal_embeddings += panoramic_embeddings
    ...
    def _compute_deepstack_embeds(
        ...
        pano_token_id = getattr(self.config, 'panoramic_image_token_id', None)
        ds_placeholder_ids = [self.config.image_token_id, self.config.video_token_id]
        if pano_token_id is not None and pano_token_id not in ds_placeholder_ids:
            ds_placeholder_ids.append(pano_token_id)
        deepstack_input_embeds = merge_multimodal_embeddings(
            input_ids,
            deepstack_input_embeds,
            multimodal_embeddings_multiscale,
            placeholder_token_id=ds_placeholder_ids,
        )
    ...
    def get_input_embeddings(
        ...
        if multimodal_embeddings is not None:
            ...
            pano_token_id = getattr(self.config, 'panoramic_image_token_id', None)
            placeholder_ids = [self.config.image_token_id, self.config.video_token_id]
            if pano_token_id is not None and pano_token_id not in placeholder_ids:
                placeholder_ids.append(pano_token_id)
