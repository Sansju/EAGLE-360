...
class GPUModelRunner(LoRAModelRunnerMixin, KVConnectorModelRunnerMixin):
    ...
    def _init_mrope_positions(self, req_state: CachedRequestState):
        ...
        panoramic_grid_thw = []  # Add panoramic support
        ...
        for mm_feature in req_state.mm_features:
            ...
            if (t := mm_input.get("panoramic_grid_thw")) is not None:
                panoramic_grid_thw.append(t.tolist())
            if mm_input.get("use_audio_in_video") is True:
                use_audio_in_video = True

        if supports_mrope(self.model):
            result = self.model.get_mrope_input_positions(
            ...
            req_state.mrope_positions = result[0]
            req_state.mrope_position_delta = result[1]
        else:
            result = MRotaryEmbedding.get_input_positions_tensor(
                req_state.prompt_token_ids,
                hf_config=self.model_config.hf_config,
                image_grid_thw=image_grid_thw,
                video_grid_thw=video_grid_thw,
                ...
                audio_feature_lengths=audio_feature_lengths,
                use_audio_in_video=use_audio_in_video,
                panoramic_grid_thw=panoramic_grid_thw if panoramic_grid_thw else None,
            )
            req_state.mrope_positions = result[0]
            req_state.mrope_position_delta = result[1]
            # Store panoramic info for rope rolling
            if len(result) > 2:
                req_state.panoramic_pos_mask = result[2]
                req_state.panoramic_width_map = result[3]
    ...
    def _calc_mrope_positions(self, scheduler_output: "SchedulerOutput"):
        mrope_pos_ptr = 0
        # Pre-compute total scheduled tokens for batch-level panoramic mask
        total_scheduled = sum(
            scheduler_output.num_scheduled_tokens[rid]
            for rid in self.input_batch.req_ids
        )
        batch_pano_mask = torch.zeros(total_scheduled, dtype=torch.bool)
        batch_pano_widths = torch.zeros(total_scheduled, dtype=torch.long)
        has_panoramic = False
        ...
        for index, req_id in enumerate(self.input_batch.req_ids):
            ...
            if prompt_part_len > 0:
                ...
                # Copy panoramic mask/widths for prompt part of this request
                pano_mask = getattr(req, 'panoramic_pos_mask', None)
                pano_widths = getattr(req, 'panoramic_width_map', None)
                if pano_mask is not None and pano_widths is not None:
                    src_len = min(prompt_part_len,
                                  len(pano_mask) - src_start)
                    if src_len > 0:
                        batch_pano_mask[dst_start:dst_start + src_len] = (
                            pano_mask[src_start:src_start + src_len])
                        batch_pano_widths[dst_start:dst_start + src_len] = (
                            pano_widths[src_start:src_start + src_len])
                        if pano_mask[src_start:src_start + src_len].any():
                            has_panoramic = True
        ...
        # NOTE: Panoramic rope rolling is intentionally disabled.
        # The model was trained with transformers where the rolling write-back
        # (cos_per_head[b,:,pano_pos,:][:,:,freq_idx] = new_val) is a silent no-op
        # because PyTorch advanced indexing returns a copy, not a view.
        # The model therefore learned standard MRoPE for panoramic tokens.
        # Enabling rolling contradicts training and reduces bFOV accuracy.
        from vllm.model_executor.layers.rotary_embedding.mrope import (
            _PANORAMIC_CACHE)
        _PANORAMIC_CACHE["pos_mask"] = None
        _PANORAMIC_CACHE["width_map"] = None
        _PANORAMIC_CACHE["positions"] = None
        _PANORAMIC_CACHE["_rope_printed"] = False
