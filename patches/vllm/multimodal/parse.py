...
class PanoramicImageProcessorItems(ProcessorBatchItems[HfImageItem]):
    """Processor items for panoramic images - uses 'panoramic_image' as modality key."""

    def __init__(self, data: Optional[Sequence[HfImageItem]]) -> None:
        if data is None:
            data = [None]
        super().__init__(data, "panoramic_image")
...
class MultiModalDataParser:
    ...
    def _parse_panoramic_image_data(
        self,
        data: ModalityData[ImageItem],
    ) -> Optional[ModalityDataItems[Any, Any]]:
        """Parse panoramic image data with correct modality key."""
        if data is None:
            return PanoramicImageProcessorItems(None)

        if self._is_empty(data):
            return None

        if self._is_embeddings(data):
            # For embeddings, use the standard ImageEmbeddingItems but this is rare
            return ImageEmbeddingItems(data)
        ...
        return PanoramicImageProcessorItems(data_items)
    ...
    def _get_subparsers(self) -> Mapping[str, ModalityDataParser]:
        return {
            ...
            "panoramic_image": self._parse_panoramic_image_data,
        }
